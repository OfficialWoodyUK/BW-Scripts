...
RegisterNetEvent('RealisticVehicleFailure:client:CleanVehicle')
AddEventHandler('RealisticVehicleFailure:client:CleanVehicle', function()
...
function cleanVehicle(veh)
    ...
    local vehicleCleaned = false

    ESX.Progressbar("Cleaning Vehicle", cfg.cleaningTime, { -- this line is where you edit how long it takes to clean the car
        disableUI = false,
        animation = {
            type = "scenario",
        ...
        onCancel = function()
            ...
            ESX.ShowNotification('Cleaning process cancelled', 'error', 2500)
            ClearPedTasks(ped)
        end
    })

    Citizen.CreateThread(function()
        while not vehicleCleaned do
            Citizen.Wait(0)
        end

        if vehicleCleaned then
            TriggerServerEvent('RealisticVehicleFailure:removecleaningkit')
            ESX.TriggerServerCallback('RealisticVehicleFailure:checkCleaningKit', function(hasCleaningKit)
                if hasCleaningKit then
                    ESX.ShowNotification('Successfully cleaned vehicle!', 'success', 2500)
                    TriggerEvent('RealisticVehicleFailure:client:SyncWash', veh)
                else
                    ESX.ShowNotification('You don\'t have a cleaning kit!', 'error', 2500)
                end
                ClearPedTasks(ped)
            end)
        end
...
local function repairVehicle(veh, engineHealth, itemName)
    local mechCount = lib.callback.await('RealisticVehicleFailure:mechCount', mechCount)
    if mechCount <= 0 then
        openVehicleDoors(veh)
        ESX.Progressbar("Repairing Vehicle", cfg.repairTime, {
            FreezePlayer = true,
            animation = {
                type = "Scenario",
            ...
            onFinish = function()
                ESX.ShowNotification("You have repaired the vehicle.")
            ...
            onCancel = function()
                ESX.ShowNotification("Repair cancelled.")
            end
        })
    else
        ESX.ShowNotification("There are too many mechanics on duty! Call someone.")
    end
end
...
local function getVehicleToRepair()
	if cache.vehicle then
		ESX.ShowNotification("Can Not Repair Vehicle From Inside")
		return
	end

	local veh = lib.getClosestVehicle(GetEntityCoords(cache.ped), 5, false)
	if not veh then
		ESX.ShowNotification("Your Not Near A Vehicle..")
		return
	end
...
RegisterNetEvent('RealisticVehicleFailure:client:RepairVehicle', function()
	...
	if engineHealth >= 500 then
		ESX.ShowNotification("You kicked the engine and it magically came back to life")
		return
	end
